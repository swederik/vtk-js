//VTK::System::Dec

/*=========================================================================

  Program:   Visualization Toolkit
  Module:    vtkPolyDataFS.glsl

  Copyright (c) Ken Martin, Will Schroeder, Bill Lorensen
  All rights reserved.
  See Copyright.txt or http://www.kitware.com/Copyright.htm for details.

     This software is distributed WITHOUT ANY WARRANTY; without even
     the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
     PURPOSE.  See the above copyright notice for more information.

=========================================================================*/

// the output of this shader
//VTK::Output::Dec

//VTK::NumberOfVolumes::Dec


varying vec3 vertexVCVSOutputArr[numVolumes];

// camera values
uniform float camThick;
uniform float camNear;
uniform float camFar;

// values describing the volume geometry
uniform vec3 vOriginVCArr[numVolumes];
uniform vec3 vSizeArr[numVolumes];
uniform vec3 vPlaneNormal0Arr[numVolumes];
uniform float vPlaneDistance0Arr[numVolumes];
uniform vec3 vPlaneNormal1Arr[numVolumes];
uniform float vPlaneDistance1Arr[numVolumes];
uniform vec3 vPlaneNormal2Arr[numVolumes];
uniform float vPlaneDistance2Arr[numVolumes];
uniform vec3 vPlaneNormal3Arr[numVolumes];
uniform float vPlaneDistance3Arr[numVolumes];
uniform vec3 vPlaneNormal4Arr[numVolumes];
uniform float vPlaneDistance4Arr[numVolumes];
uniform vec3 vPlaneNormal5Arr[numVolumes];
uniform float vPlaneDistance5Arr[numVolumes];

// opacity and color textures
uniform sampler2D otextureArr[numVolumes];
uniform float oshiftArr[numVolumes];
uniform float oscaleArr[numVolumes];
uniform sampler2D ctextureArr[numVolumes];
uniform float cshiftArr[numVolumes];
uniform float cscaleArr[numVolumes];

// jitter texture
uniform sampler2D jtexture;

// some 3D texture values
uniform highp sampler3D texture1[numVolumes];
uniform float sampleDistance;
uniform vec3 vVCToIJK[numVolumes];

// declaration for intermixed geometry
//VTK::ZBuffer::Dec

// Lighting values
//VTK::Light::Dec

// normal calc
//VTK::Normal::Dec

// gradient opacity
//VTK::GradientOpacity::Dec

vec2 getRayPointIntersectionBounds(
vec3 rayPos, vec3 rayDir,
vec3 planeDir, float planeDist,
vec2 tbounds, vec3 vPlaneX, vec3 vPlaneY,
float vSize1, float vSize2, vec3 vOrigin)
{
  float result = dot(rayDir, planeDir);
  if (result == 0.0)
  {
    return tbounds;
  }
  result = -1.0 * (dot(rayPos, planeDir) + planeDist) / result;
  vec3 xposVC = rayPos + rayDir*result;
  vec3 vxpos = xposVC - vOrigin;
  vec2 vpos = vec2(
  dot(vxpos, vPlaneX),
  dot(vxpos, vPlaneY));

  // on some apple nvidia systems this does not work
  // if (vpos.x < 0.0 || vpos.x > vSize1 ||
  //     vpos.y < 0.0 || vpos.y > vSize2)
  // even just
  // if (vpos.x < 0.0 || vpos.y < 0.0)
  // fails
  // so instead we compute a value that represents in and out
  //and then compute the return using this value
  float xcheck = max(0.0, vpos.x * (vpos.x - vSize1)); //  0 means in bounds
  float check = sign(max(xcheck, vpos.y * (vpos.y - vSize2))); //  0 means in bounds, 1 = out

  return mix(
  vec2(min(tbounds.x, result), max(tbounds.y, result)), // in value
  tbounds, // out value
  check);  // 0 in 1 out
}

vec2 getTextureBounds(vec3 vertexVCVSOutput, vec3 rayDir,
vec3 vPlaneNormal0, float vPlaneDistance0,
vec3 vPlaneNormal1, float vPlaneDistance1,
vec3 vPlaneNormal2, float vPlaneDistance2,
vec3 vPlaneNormal3, float vPlaneDistance3,
vec3 vPlaneNormal4, float vPlaneDistance4,
vec3 vPlaneNormal5, float vPlaneDistance5,
vec2 tbounds, vec3 vSize, vec3 vOrigin) {
  // all this is in View Coordinates
  tbounds = getRayPointIntersectionBounds(vertexVCVSOutput, rayDir,
  vPlaneNormal0, vPlaneDistance0, tbounds, vPlaneNormal2, vPlaneNormal4,
  vSize.y, vSize.z, vOrigin);
  tbounds = getRayPointIntersectionBounds(vertexVCVSOutput, rayDir,
  vPlaneNormal1, vPlaneDistance1, tbounds, vPlaneNormal2, vPlaneNormal4,
  vSize.y, vSize.z, vOrigin);
  tbounds = getRayPointIntersectionBounds(vertexVCVSOutput, rayDir,
  vPlaneNormal2, vPlaneDistance2, tbounds, vPlaneNormal0, vPlaneNormal4,
  vSize.x, vSize.z, vOrigin);
  tbounds = getRayPointIntersectionBounds(vertexVCVSOutput, rayDir,
  vPlaneNormal3, vPlaneDistance3, tbounds, vPlaneNormal0, vPlaneNormal4,
  vSize.x, vSize.z, vOrigin);
  tbounds = getRayPointIntersectionBounds(vertexVCVSOutput, rayDir,
  vPlaneNormal4, vPlaneDistance4, tbounds, vPlaneNormal0, vPlaneNormal2,
  vSize.x, vSize.y, vOrigin);
  tbounds = getRayPointIntersectionBounds(vertexVCVSOutput, rayDir,
  vPlaneNormal5, vPlaneDistance5, tbounds, vPlaneNormal0, vPlaneNormal2,
  vSize.x, vSize.y, vOrigin);

  return tbounds
}

void main()
{
  float scalar;
  vec4 scalarComps;

  // -- Step 1: Obtain bounds for each volume

  // Find the texture bounds for each volume based on the current
  // camera clipping plane dimensions
  vec2 tboundsPerVolume[numVolumes];
  vec3 startVPosPerVolume[numVolumes];
  vec3 endVPosPerVolume[numVolumes];
  vec3 rayDirPerVolume[numVolumes];

  for (int n = 0; n < numVolumes ; ++n)
  {
    vec3 vPlaneNormal0 = vPlaneNormal0Arr[n];
    vec3 vPlaneNormal1 = vPlaneNormal1Arr[n];
    vec3 vPlaneNormal2 = vPlaneNormal2Arr[n];
    vec3 vPlaneNormal3 = vPlaneNormal3Arr[n];
    vec3 vPlaneNormal4 = vPlaneNormal4Arr[n];
    vec3 vPlaneNormal5 = vPlaneNormal5Arr[n];
    vec3 vertexVCVSOutput = vertexVCVSOutputArr[n];
    vec3 vOriginVC = vOriginVCArr[n];
    vec3 vSize = vSizeArr[n];
    sampler2D otexture = otextureArr[n];
    float oshift = oshiftArr[n];
    float oscale = oscaleArr[n];
    sampler2D ctexture = ctextureArr[n];
    float cshift = cshiftArr[n];
    float cscale = cscaleArr[n];

    // camera is at 0,0,0 so rayDir for perspective is just the vc coord
    vec3 rayDir = normalize(vertexVCVSOutput);
    vec2 tbounds = vec2(100.0*camFar, -1.0);

    tbounds = getTextureBounds(tbounds, vertexVCVSOutput, rayDir,
    vPlaneNormal0, vPlaneDistance0,
    vPlaneNormal1, vPlaneDistance1,
    vPlaneNormal2, vPlaneDistance2,
    vPlaneNormal3, vPlaneDistance3,
    vPlaneNormal4, vPlaneDistance4,
    vPlaneNormal5, vPlaneDistance5,
    tbounds, vSize, vOriginVC)

    // do not go behind front clipping plane
    tbounds.x = max(0.0, tbounds.x);

    // do not go PAST far clipping plane
    float farDist = -camThick/rayDir.z;
    tbounds.y = min(farDist, tbounds.y);

    tboundsPerVolume[n] = tbounds;

    // We can skip some volumes
    if (tbounds.y < tbounds.x) {
      continue;
    }

    // Do not go past the zbuffer value if set
    // This is used for intermixing opaque geometry
    //VTK::ZBuffer::Impl

    // compute starting and ending values in volume space
    vec3 startVC = vertexVCVSOutput + tbounds.x*rayDir;
    startVC = startVC - vOriginVC;

    // vpos and endvpos are in VolumeCoords not Index yet
    vec3 vpos = vec3(
    dot(startVC, vPlaneNormal0),
    dot(startVC, vPlaneNormal2),
    dot(startVC, vPlaneNormal4));
    vec3 endVC = vertexVCVSOutput + tbounds.y*rayDir;
    endVC = endVC - vOriginVC;
    vec3 endvpos = vec3(
    dot(endVC, vPlaneNormal0),
    dot(endVC, vPlaneNormal2),
    dot(endVC, vPlaneNormal4));

    startVPosPerVolume[n] = vpos;
    endVPosPerVolume[n] = endvpos;
    rayDirPerVolume[n] = rayDir;

  }

  // TODO:
  // - Get min/max dimensions for the start and end vpos across volumes?
  //   - What if they have different ray directions?
  //   - Use this to determine vdelta (distance to traverse along the ray at each step)
  vec3 endvpos = endVPosPerVolume[0];
  vec3 vpos = startVPosPerVolume[0];

  // -- Step 2: Run sampling / compositing loop
  vec3 vdelta = endvpos - vpos;
  float numSteps = length(vdelta) / sampleDistance;
  vdelta = vdelta / float(numSteps);

  // start slightly inside and apply some jitter
  float jitter = texture2D(jtexture, gl_FragCoord.xy/32.0).r;
  vpos = vpos + vdelta*(0.01 + 0.98*jitter);
  vec4 color = vec4(0.0, 0.0, 0.0, 0.0);

  // TODO: Right now this uses [0]! - What should we be doing here??
  vec3 ijk = vpos * vVCToIJK[0];
  vdelta = vdelta * vVCToIJK[0];
  int count = int(numSteps - 0.2); // end slightly inside
  for (int i = 0; i < //VTK::MaximumSamplesValue ; ++i)
  {
    // compute the scalar
    scalar = texture(texture1, ijk).r;

    // now map through opacity and color
    vec4 tcolor = texture2D(ctexture, vec2(scalar * cscale + cshift, 0.5));
    tcolor.a = texture2D(otexture, vec2(scalar * oscale + oshift, 0.5)).r;

    // compute the normal if needed
    //VTK::Normal::Impl

    // handle gradient opacity
    //VTK::GradientOpacity::Impl

    // handle lighting
    //VTK::Light::Impl

    float mix = (1.0 - color.a);

    color = color + vec4(tcolor.rgb*tcolor.a, tcolor.a)*mix;
    if (color.a > 0.99) { color.a = 1.0; break; }
    if (i >= count) { break; }
    ijk += vdelta;
  }

  gl_FragData[0] = vec4(color.rgb/color.a, color.a);
  // gl_FragData[0] = vec4(tbounds.y/farDist, tbounds.x/farDist, color.b/color.a, 1.0);
}
